...
public:
    ...
    FG_eval(Eigen::VectorXd coeffs, double limit)
    {
        this->coeffs = coeffs;
        this->limit = limit;
    }
    ...
    void operator()(ADvector& fg, const ADvector& vars)
    {
        ...
        for (int i = 0; i < N; i++)
        {
            fg[0] += 2.0*CppAD::pow(vars[cte_start + i] - ref_cte, 2); // This is like the P term in a PID, make it strong to turn in quickly
        ...
        for (int i = 0; i < N - 1; i++)
        {
            fg[0] += 2.0e-4*vars[v_start +i]*CppAD::pow(vars[delta_start + i], 2);
        ...
        for (int i = 0; i < N - 2; i++)
        {
            fg[0] += 2.0e+4*vars[v_start + i]*CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
...
// Helper Function to go from global to local coordinates
Eigen::MatrixXd MPC::Transform(vector<double> ptsx, vector<double> ptsy, Eigen::VectorXd state)
{
    ...
    Eigen:: MatrixXd out_vec(3, ptsx.size());
    out_vec = R * T * wps;
